    ...
    <div className="flex h-screen text-white font-sans overflow-hidden">
      ...
      `}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 pb-20 md:pb-4 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
            <div className={`text-xs font-bold text-text-secondary uppercase mb-2 px-2 ${!sidebarOpen && 'md:block'}`}>Main</div>
        ...
        <div className="p-4 border-t border-white/5">
            <Link 
                ...
                className="flex items-center gap-3 mb-4 px-2 p-2 rounded-xl border border-transparent hover:bg-white/5 hover:border-white/10 transition-all group"
            >
                <div className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center text-xs font-bold group-hover:scale-110 transition-transform shrink-0">
                    {user?.email?.[0].toUpperCase() || 'U'}
                </div>
                <div className={`flex-1 overflow-hidden transition-opacity ${sidebarOpen ? 'opacity-100' : 'opacity-100 block md:block'}`}>
                    <div className="text-sm font-bold truncate group-hover:text-primary transition-colors">{user?.user_metadata?.full_name || 'User'}</div>
