...
async def submit_result(task_id: int, file: UploadFile = File(...), db: Session = Depends(database.get_db), key: str = Depends(verify_worker_key)):
    ...
    final_video_url = f"/items/{task_id}_{file.filename}" # Fallback
    
    # Upload to permanent CDN for Cross-Device Support
    try:
        ...
        if catbox_resp.status_code == 200:
            final_video_url = catbox_resp.text.strip()
    ...
    return {"status": "success", "file_path": file_location, "video_url": final_video_url}
